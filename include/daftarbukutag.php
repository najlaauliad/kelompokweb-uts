...
if (isset($_SESSION['tag'])) {
    $data_tag = $_SESSION['tag'];
    $sql_k = "SELECT `tag` FROM `tag` WHERE `id_tag`= '$data_tag'";
...
<section id="katalog-item">
    <main role="main" class="container">
        ...
        if (isset($_SESSION['tag'])) {
            ...
            <?php } ?>
        <?php } else { ?>
            <h2 class="text-primary">TAG: ALL DATA</h2><br><br>
        <?php } ?>
        <div class="row">
            <div class="col-md-9 katalog-main">
                <div class="row">
                    ...
                    $sql_jum = "SELECT DISTINCT `b`.`id_buku`, `b`.`judul`, `b`.`cover`,`p`.`penerbit` FROM `buku` `b` INNER JOIN `penerbit` `p` ON `b`.`id_penerbit` = `p`.`id_penerbit` INNER jOIN `tag_buku` `u` ON `b`.`id_buku` = `u`.`id_buku`";
                    if (!empty($data_tag)) {
                        $sql_jum .= " WHERE `u`.`id_tag` = '$data_tag'";
                    }
                    ...
                    <!-- getdata -->
                    <?php
                    $sql_b = "SELECT DISTINCT `b`.`id_buku`, `b`.`judul`, `b`.`cover`,`p`.`penerbit` FROM `buku` `b` INNER JOIN `penerbit` `p` ON `b`.`id_penerbit` = `p`.`id_penerbit` INNER jOIN `tag_buku` `u` ON `b`.`id_buku` = `u`.`id_buku` ";
                    //echo $sql_b;
                    if (!empty($data_tag)) {
                        $sql_b .= " WHERE `u`.`id_tag` = '$data_tag'";
                    }
                ...
                <ul class="pagination pagination-sm m-0 float-right">
                    ...
                    if ($jum_halaman == 0) {
                        ...
                        $setelah = $halaman + 1;
                        if ($halaman != 1) {
                            echo "<li class='page-item'><a class='page-link'href='index.php?include=daftar-buku-tag&halaman=1'>First</a></li>";
                            echo "<li class='page-item'><a class='page-link'href='index.php?include=daftar-buku-tag&halaman=$sebelum'>«</a></li>";
                        }
                        for ($i = 1; $i <= $jum_halaman; $i++) {
                            if ($i > $halaman - 5 and $i < $halaman + 5) {
                                if ($i != $halaman) {
                                    echo "<li class='page-item'><a class='page-link'href='index.php?include=daftar-buku-tag&halaman=$i'>$i</a></li>";
                                } else {
                        ...
                        if ($halaman != $jum_halaman) {
                            echo "<li class='page-item'><a class='page-link'href='index.php?include=daftar-buku-tag&halaman=$setelah'>»</a></li>";
                            echo "<li class='page-item'><a class='page-link'href='index.php?include=daftar-buku-tag&halaman=$jum_halaman'>Last</a></li>";
                        }
                    } ?>
                </ul>
            </div><!-- /.katalog-main -->
